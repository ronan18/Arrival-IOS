...
public class ToStationIntentHandler: NSObject, TrainsToStationIntentHandling {
    ...
    public func resolveDestination(for intent: TrainsToStationIntent) async -> IntentStationResolutionResult {
        print("resolve destionation intent")

        return IntentStationResolutionResult.success(with: intent.destination!)
        
    }
    
    
    public func provideDestinationOptionsCollection(for intent: TrainsToStationIntent, searchTerm: String?, with completion: @escaping (INObjectCollection<IntentStation>?, Error?) -> Void) {
    ...
    public func handle(intent: TrainsToStationIntent, completion: @escaping (TrainsToStationIntentResponse) -> Void) {
        guard let destination = intent.destination else {
            completion(TrainsToStationIntentResponse.failure(destination: IntentStation(identifier: "", display: "")))
            return
        }
        print("handling to trains intent")
        completion(TrainsToStationIntentResponse.success(destination: destination))
    }
}
